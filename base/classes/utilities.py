...
config_directory = join(root_directory, "base", "config")
cogs_directory = join(root_directory, "base", "cmds")
...
async def cogs_manager(bot: SpinovelBot, mode: str, cogs: list[str]) -> None:
    for cog in cogs:
        try:
            if mode == "unload":
...
def set_logging(file_level: int = logging.DEBUG, console_level: int = logging.INFO, filename: str = "discord.log") -> \
...
def bot_has_permissions(**perms: bool):
    ...
    def wrapped(command: Union[app_commands.Command, commands.HybridCommand, commands.Command]) -> Union[
        app_commands.Command, commands.HybridCommand, commands.Command]:
