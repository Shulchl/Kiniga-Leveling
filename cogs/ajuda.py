...
async def send_embed(ctx, embed):
    try:
        ...
        try:
            await ctx.send("Parece que eu não consigo enviar embeds, por favor, verifique minhas permissões! :)")
        except Forbidden:
            await ctx.author.send(
                f"Parece que eu não consigo enviar mensagens em {ctx.guild.name}\n"
                f"Teria como avisar o exelentíssimo Shuichi, sobre isso? :slight_smile: ", embed=embed)
            
class Ajuda(commands.Cog, name="Ajuda", description="Mostra essa mesma mensagem."):
    ...
    async def ajuda(self, ctx, *module):
        module = " ".join(module)
