...
class Mod(commands.Cog):
    ...
    async def createMold(self, ctx, type, name, valor, img_loja, img_perfil, img_round_title, nivel, canbuy):
        await asyncsleep(4)
        try:
            await self.db.fetch(f"INSERT INTO itens (name, type, value, img, img_profile, imgd, lvmin, canbuy) VALUES (\'{name}\', \'{type}\', {valor}, \'{img_loja}\', \'{img_perfil}\', \'{img_round_title}\', {nivel}, {canbuy}) ON CONFLICT (name) DO NOTHING")
    ...
    async def createItem(self, ctx, type, name, valor, img_loja, nivel, canbuy):
        await asyncsleep(4)
        try:
            await self.db.fetch(f"INSERT INTO itens (name, type, value, img, lvmin, canbuy) VALUES (\'{name}\', \'{type}\', {valor}, \'{img_loja}\', {nivel}, {canbuy}) ON CONFLICT (name) DO NOTHING")
        except Exception as i:
            await ctx.send(i)
    ...
    @commands.has_permissions(administrator=True)
    async def citem(self, ctx, type, name, valor: int, nivel: int, img_loja, img_perfil = None, img_round_title = None, canbuy: bool = True) -> None:
        msg = await ctx.reply(f"Deseja inserir {type} {name}, por {valor} oris, na loja?")
        verify = await self.bot.wait_for('message', timeout=10, check= lambda i: i.author == ctx.author and i.content in ['s', 'sim', 'Sim', 'SIM', 'n', 'não', 'Não'])
        try:
    ...
    async def on_raw_reaction_add(self, payload:discord.RawReactionActionEvent):
        ...
        if payload.message_id == giveway_idFunction.ids and payload.user_id != self.bot.user.id:
            ...
            try:
                ...
                itens = inv[0][0].split(',')
                j = [[t[0][0], t[0][1]] for t in [(await self.db.fetch(f"SELECT id, name FROM itens WHERE id=('{i}')")) for i in itens] if str(t[0][1]) == 'Ticket']
